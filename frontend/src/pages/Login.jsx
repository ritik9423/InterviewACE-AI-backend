...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.success('Welcome back to the elite!', { icon: '✨' });
            navigate('/dashboard');
        } catch (error) {
            ...
            >
                <div className="glass-thick p-8 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl border border-white/10 relative overflow-hidden">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <button 
                            disabled={isLoading}
                            className="w-full py-4 bg-white text-black font-black text-lg rounded-xl hover:bg-slate-100 transition-all shadow-2xl active:scale-[0.98] mt-4 disabled:opacity-50"
                        >
                    ...
                    <p className="mt-10 text-center text-slate-500 text-sm font-medium">
                        New candidate? <Link to="/signup" className="text-indigo-400 font-black hover:text-indigo-300 transition-colors">Register Identity</Link>
                    </p>
                </div>
            </motion.div>
        </div>
